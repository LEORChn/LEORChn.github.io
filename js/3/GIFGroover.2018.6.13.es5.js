/*
	fork from     https://github.com/blindman67/GIFGroover
	fork version  2018-6-13
	fork date     2019-11-19
	last modified 2021-8-15
*/
function GIFGroover() {
    "use strict";
    ...
    function decodePixels(minSize, data) {
        ...
        while (busy) { 
            ...
            if (key === clear) { // reset the dictionary
                ...
                if (key >= len) {
                	var dic_temp = cloneArray(dic[last]);
                	dic_temp.push(dic[last][0]);
                	dic[len ++] = dic_temp; //[...dic[last], dic[last][0]]
                } else if (last !== clear) {
                	var dic_temp = cloneArray(dic[last]);
                	dic_temp.push(dic[key][0]);
                	dic[len ++] = dic_temp; //[...dic[last], dic[key][0]]
                }
                plen = dic[key].length;
    ...
    function startLoad(filename) {
        if (gifSrc === undefined) {
            ...
            gifSrc = filename;
            setTimeout(function(){ loadGif(gifSrc) }, 0);
        } else {
    ...
    function fireEvent(name, data, clearEvent) {
    ...
    const gif = {                               // the gif image object
        ...
        get allFrames()    { return frames.map(function(frame){ return frame.image } ) },  // returns array of frames as images (canvas).
        ...
        getFrame(index) {                           // return the frame at index 0. If index is outside range closet first or last frame is returned
            //     frames[index < 0 ? 0 : index >= frames.length ? frames.length-1 : index].image;
            return frames[Math.min(Math.max(0, parseInt(index)), frames.length - 1)].image;
        },
