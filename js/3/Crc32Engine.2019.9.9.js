// fork from https://github.com/blogwy/BilibiliVideoDownload/commit/57b0a54a9aac8c6325ed57d7ffeea9097fb897ca
// referrer  https://blog.wangyu.link/2019/09/12/2019-09-12/
// origin    2019-9-9
// fork on   2021-8-23
// modified  2021-8-23
...
class Crc32Engine{
	...
	constructor(){
		...
		let runningSum = 0;
		this.shortHashBucketStarts = shortHashBuckets.map(function(n){
			return runningSum += n
		});
		for(let i = 0; i < 100000; i++){
		...
		for(let digitCount = 1; digitCount < 10; digitCount++){
			...
			if(digitCount < 6){
				...
				for(let prefix = startPrefix; prefix < endPrefix; prefix++){
					for(let postfix of this.lookup(hashVal ^ baseHash ^ this.compute(prefix, true))){
...
if(typeof module != 'undefined') module.exports = Crc32Engine;
